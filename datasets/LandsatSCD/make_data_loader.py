...
import VCFN.datasets.LandsatSCD.imutils as imutils
...
class Datset(Dataset):
    ...
    def __transforms(self, aug, pre_img, post_img, label):
        if aug:
            pre_img, post_img, label = imutils.random_crop_mcd(pre_img, post_img, label, self.crop_size)
            pre_img, post_img, label = imutils.random_fliplr_mcd(pre_img, post_img, label)
            pre_img, post_img, label = imutils.random_flipud_mcd(pre_img, post_img, label)
            pre_img, post_img, label = imutils.random_rot_mcd(pre_img, post_img, label)
    ...
    def __getitem__(self, index):
        ...
        if 'train' in self.data_pro_type:
            ...
            pre_img, post_img, cd_label = self.__transforms(False, pre_img, post_img, label)
