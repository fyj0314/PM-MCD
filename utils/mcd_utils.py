...
def eval_building(preds, labels, num_class):
    hist = np.zeros((num_class, num_class))
    ...
    # hist[:, 25] += hist[:, 27]
    # hist[:, 26] += hist[:, 30]
    classes_to_extract = [0, 5, 11, 17, 23, 25, 26, 27, 28, 30, 35]
    hist = hist[np.ix_(classes_to_extract, classes_to_extract)]
    hist_fg = hist[1:, 1:]
    c2hist = np.zeros((2, 2))
    c2hist[0][0] = hist[0][0]
    ...
    # 计算每个类别的miou
    # Mious = np.zeros(num_class)
    # for i in range(num_class):
    #     TP = hist[i, i]
    #     FP = hist.sum(1)[i] - hist[i][i]
    #     FN = hist.sum(0)[i] - hist[i][i]
    #     Mious[i] = TP / (FP + FN + TP)
    Ious = np.diag(hist) / (hist.sum(1) + hist.sum(0) - np.diag(hist))
    OA = np.diag(hist).sum() / hist.sum()
    return kappa_n0, Fscd, IoU_mean, Sek, SC_Precision, SC_Recall, Ious, OA

def eval_building4(preds, labels, num_class):
    hist = np.zeros((num_class, num_class))
    for pred, label in zip(preds, labels):
        infer_array = np.array(pred)
        unique_set = set(np.unique(infer_array))
    ...
    # hist[:, 25] += hist[:, 27]
    # hist[:, 26] += hist[:, 30]
    classes_to_extract = [0, 3, 6, 9, 10, 11, 12]
    hist = hist[np.ix_(classes_to_extract, classes_to_extract)]
    hist_fg = hist[1:, 1:]
    c2hist = np.zeros((2, 2))
    c2hist[0][0] = hist[0][0]
...
def eval_building3(preds, labels, num_class):
    hist = np.zeros((num_class, num_class))
    for pred, label in zip(preds, labels):
        infer_array = np.array(pred)
        unique_set = set(np.unique(infer_array))
...
def SCDD_eval_all(preds, labels, num_class):
    ...
    return kappa_n0, Fscd, IoU_mean, Sek#, SC_Precision, SC_Recall, Mious
